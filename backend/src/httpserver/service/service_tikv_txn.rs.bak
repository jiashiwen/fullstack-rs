...
pub async fn s_txn_put(put: KV) -> Result<()> {
    let tikvhandler = get_tikv_handler().await;
    let result = tikvhandler.txn_put(put.Key, put.Value).await.map_err(|e| {
        return GlobalError::from_err(e.to_string(), GlobalErrorType::UnknowErr);
    })?;
    Ok(result)
}

pub async fn s_txn_get(key: String) -> Result<String> {
    let tikvhandler = get_tikv_handler().await;
    let result = tikvhandler.txn_get(key).await.map_err(|e| {
        return GlobalError::from_err(e.to_string(), GlobalErrorType::UnknowErr);
    })?;
...
//
// pub async fn s_txn_flush_all() -> Result<()> {
//     let tikvhandler = get_tikv_handler().await;
...
// }
//
// pub async fn s_txn_scan(begin: String, end: String, limited: u32) -> Result<Vec<KV>> {
//     let tikvhandler = get_tikv_handler().await;
